...
5. `summarise()` sends everything to Claude (`claude-haiku-4-5`, `max_tokens=16000`, `thinking={"type": "enabled", "budget_tokens": 2000}`) using `SYSTEM_PROMPT` which strictly defines five sections (umbrella / weather / new emails / follow up / this week) with **no markdown** — the PWA renders plain text + emoji headers only.
